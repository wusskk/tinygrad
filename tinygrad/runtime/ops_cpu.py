...
class ClangJITCompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    ...
    target = ['--target=x86_64-none-win32-elf'] if sys.platform == 'win32' else [f'--target={platform.machine()}-none-unknown-elf', '-fPIC']
    args = ['-march=native', *target, '-O2', '-ffreestanding', '-fno-math-errno', '-nostdlib']
